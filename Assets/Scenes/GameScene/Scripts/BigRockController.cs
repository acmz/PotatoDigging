...
public class BigRockController : MonoBehaviour {
    ...
    // 点数
    private const int SCORE_POINT = 500;

    // 体力
    private int _lifePoint = 10;
    ...
    // 小岩放出数
    private const int SPRIT_LOCK_GENERATE_NUM = 5;

    // 小岩放出処理判定フラグ
    private bool _generateSpritRockProgress = false;

    private void GenerateSpritRock() {

        if(this._generateSpritRockProgress) {
            return;
        }

        this._generateSpritRockProgress = true;

        // 小岩を放出する
        Vector2 spritRockGeneratePos = gameObject.transform.position;
        for(int loopCounter = 0;loopCounter < SPRIT_LOCK_GENERATE_NUM;loopCounter++) {
            GameObject spritRockObject = GameObject.Find("Sprit_Rock_Generator");
            spritRockObject.GetComponent<SpritRockGenerator>().GenerateRock(spritRockGeneratePos);
        }
    ...
    public void RockShoot(Vector2 inRockGeneratePos) {
        ...
        Vector2 endPos = new Vector2(ROCK_DESTROY_POS_LEFT, rockDestroyPosY);
